...
SIG=sum(var(X));   % Not required for this model
var_prior=SIG*10;
...
[predictions,  relevant_points] = fit_rvc (X, w, nu, X_test, initial_psi, @kernel_gauss, LAMBDA);

relevant_points=double(relevant_points); % converting symbolic to numerical values
relevant_points_num=sum(relevant_points);
